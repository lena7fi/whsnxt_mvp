...
import { StyleSheet, Image, Text, View, ImageBackground, Dimensions, ScrollView, SafeAreaView, Platform } from "react-native"
...
export default function EventDetailScreen({ navigation, route }) {
  ...
  const [coordinates, setCordinates] = useState({
    latitude: 28.579660, longitude: 77.321110
  })
  ...
  const getEventDetails = (eventId) =>{
    if(eventId && eventId !==undefined)
    {
      ...
      .then(responseData => {
        ...
        const data = responseData && responseData.data && responseData.data.length > 0 ?responseData.data[0]:null;
        setEvent(data);
        const venue = data && data.venue?data.venue: null;
        const name  = data && data.name?data.name: null;
        const edata = data && data.event_date? new Date(data.event_date): null;
  ...
  return (
    <SafeAreaView style={styles.PrivacyPolicy}>
      <ImageBackground source={event && event.cover?{uri: event.cover}:require('./back.png')} style={{ width: width, aspectRatio: 375 / 281, height: 'auto' }}>
      ...
      <View style={{ padding: 20 }}>
          <View style={{ flexDirection: 'row' }}>
            {date && date !==null && (
            <Text style={{ fontSize: 14, color: 'white' }}>{date.toLocaleUpperCase()}</Text>
            )}
            {time && time !==null && (
          ...
          {title && title !== null && (
          <Text style={{ fontSize: 16, color: 'white', fontWeight: 'bold', marginTop: 5 }}>{title}</Text>
          )}
          {artist && artist !==null && (
          <View style={{ flexDirection: 'row', marginTop: 5 }}>
            <Image source={require('./i_event.png')} />
            <Text style={{ color: 'white', fontSize: 10, marginLeft: 10 }}>Event by <Text style={{ color: 'white', fontSize: 10, fontWeight: 'bold' }}>{artist}</Text></Text>
          </View>
          )}
          {venue && venue !== null && (
          <View style={{ flexDirection: 'row', marginTop: 5 }}>
            <Image source={require('./i_location.png')} />
            <Text style={{ color: 'white', fontSize: 10, textDecorationLine: 'underline', marginLeft: 10 }}>{venue}</Text>
          </View>
          )}
