...
import { StyleSheet, Image, Text, View, ImageBackground, Dimensions, TextInput, ScrollView, SafeAreaView, ActivityIndicator, TouchableOpacity } from "react-native"
...
export default function UserSearchScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.PrivacyPolicy}>
      {loading ? <ActivityIndicator color="white" size="large" style={{ alignSelf: 'center', flex: 1 }} /> : (
      <>
      ...
      <ScrollView contentContainerStyle={{ display: 'flex', flexDirection: 'column' }}>
        {
          users.map(user => (
            <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', marginTop: 30 }}>
              <Text style={{ fontSize: 14, fontWeight: '600', color: 'white' }}>{`${user.first_name} ${user.last_name}`}</Text>
              <TouchableOpacity activeOpacity={0.5} onPress={() => navigation.navigate('Profile', {userId: user.id}) }>
