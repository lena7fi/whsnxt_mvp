...
import { StyleSheet, Image, Text, View, ImageBackground, Dimensions, TextInput, ScrollView, SafeAreaView, Platform, TouchableOpacity } from "react-native"
  ...
  return (
    <SafeAreaView style={styles.PrivacyPolicy}>
      ...
      <ScrollView contentContainerStyle={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
        <View style={{ display: 'flex', flexDirection: 'row', flexWrap:"wrap", marginTop: 10, justifyContent: 'center', alignItems: 'center' }}>
          {products?.map((item, index)=>(
          <View key={index} style={{ width: '40%', marginTop: 20, backgroundColor: 'white', borderRadius: 10, marginRight: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 10 }} >
            <TouchableOpacity onPress={() => navigation.navigate('ProductDetail', {id: userId, product: item})}>
              <Image source={item.product_data.album ? {uri: `${imageUrl+item.product_data.album}`} : require('./store1.png')} style={{ width: '90%', aspectRatio: 1, height: 'auto' }} />
              <View style={{display: 'flex', flexDirection: 'row', width: '100%', marginTop: 5}}>
                <Text style={{ fontSize: 10, fontWeight: '700', alignSelf: 'flex-start', width: '30%' }}>${item.product_data.price}</Text>
                <Text style={{ fontSize: 10, fontWeight: '700', alignSelf: 'center', justifyContent: 'center', width: '60%' }}>{item?.product_data?.audio_name && item.product_data.audio_name.length > 10 ? `${item.product_data.audio_name.substring(0, 10)}...`: item.product_data.audio_name}</Text>
              </View>
              <Text style={{ fontSize: 10, fontWeight: '700', alignSelf: 'flex-start', marginTop: 15 }}>{item?.product_data?.album_title && item?.product_data?.album_title.length > 15 ? `${item.product_data.album_title.substring(0, 15)}...`: item.product_data.album_title}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={{ position: 'absolute', right: 5, bottom: 20, marginRight: 10 }} onPress={() => addBookmark(item.product_data.id, item?.user_bookmarked)} >
