...
import { StyleSheet, Image, Text, View, ImageBackground, Dimensions, ScrollView, SafeAreaView, Platform, ActivityIndicator } from "react-native"
import { connect } from "react-redux";
import { accountUrl, postUrl } from "../../constants/BaseUrl";
...
function ProfileScreen({ navigation, userData, route }) {
  ...
  useEffect(() => {
    ...
    fetch(accountUrl, {
      ...
      if (responseData.error == false) {
        console.log(responseData["0"].id, 'id')
        let fetchFollowInfo = new FormData();
        fetchFollowInfo.append('func', 'fetch_followers');
        fetchFollowInfo.append('user_id', responseData["0"].id);
        fetch(accountUrl, {
          method: 'post',
          headers: {
            Accept: 'application/json',
          ...
          return response1.json();}).then(responseData1 => {
          setUserProfile(responseData["0"]);
          let followers = [];
          for(const item in responseData1) {
            if(!isNaN(item)) {
              followers.push(responseData1[item]);
            }
          }
  ...
  const getUserImage = (uri) => {
    if (uri != undefined && uri != '') {
      return <Image
        source={{ uri: uri }}
        style={{ alignSelf: 'center', marginTop: 10, width: 100, height: 100, borderRadius: 50 }}
      />
    } else {
      return <Image
  ...
  return (
    <SafeAreaView style={styles.PrivacyPolicy}>
      {isLoading ? <ActivityIndicator color="white" size="large" style={{ alignSelf: 'center', flex: 1 }} /> : (
        <>
          <View style={styles.Group642}>
            ...
            />
            <Text style={styles.Txt432}>{userProfile.role == 2 ? "Talent" : "Normal User"}</Text>
          ...
          {getUserImage(userProfile.profile_pic)}
          <Text style={{ fontSize: 12, fontWeight: '600', color: 'white', alignSelf: 'center', marginTop: 20 }}>{userProfile.first_name + ' ' + userProfile.last_name}</Text>
          <View style={{ display: 'flex', alignSelf: 'center' }}>
            <View style={{ alignSelf: 'flex-start', flexDirection: 'row', marginTop: 10 }}><Image source={require('./i_location.png')} style={{ alignSelf: 'center' }} /><Text style={{ fontSize: 8.26, color: 'white', marginLeft: 10 }}>United Kindom, London</Text></View>
            <View style={{ alignSelf: 'flex-start', flexDirection: 'row', marginTop: 10 }}><Image source={require('./i_position.png')} style={{ alignSelf: 'center' }} /><Text style={{ fontSize: 8.26, color: 'white', marginLeft: 10 }}>Portsoken</Text></View>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 10 }}>
            <View onTouchEnd={() => onToggleFollow(status == 1? 0: 1)} style={{ backgroundColor: '#1455F5', borderRadius: 4, paddingHorizontal: 30, paddingVertical: 10, marginHorizontal: 10 }}><Text style={{ fontSize: 12, color: 'white' }}>{status == 1? "Unfollow": "Follow"}</Text></View>
