...
import { Platform, StyleSheet, Image, Text, View, Dimensions, TextInput, ScrollView, SafeAreaView, TouchableOpacity, ActivityIndicator } from "react-native";
...
function StyledInput({ label, type, onChangeText, value, autoFocus, width, editable = false, multiline, numberOfLines }) {
  return (
...
function RenderDropdown({ items, label, value, width, placeholder, isFocus, setFocus, setValue }) {
  return (
    <View style={styles.Group4}>
      <Text style={styles.Txt066}>{label}</Text>
      <View style={{ ...styles.Rectangle17_, width: width }} />
 ...
 function EditProfileScreen({ navigation, route, setUserData, userData }) {
  ...
  useEffect(()=>{
    ...
    console.log("accountType---->", route.params?.accountType);
    const data = userData && userData !== undefined ? userData: null;
    console.log("data---->", data);
    const name = data && data.first_name?data.first_name+" "+data.last_name:'';
    const id = data && data.id?data.id:'';
    const email = data && data.email?data.email:'';
    const mobile = data && data.phone?data.phone:'';
    const gender = data && data.gender?data.gender:'';
    const gendre = data && data.gender?data.gendre:'';
    ...
    const profile = data && data.profile_pic? `${imageUrl+'profile_pic/'+data.profile_pic}` :'';
   // console.log("profile---->", imageUrl+profile);
    ...
    const updateUser = () => {
      ...
      if(filePath && filePath.length > 0)
      {
        formdata.append('profile_pic', {
          ...
          uri: Platform.OS === 'ios' ? filePath[0].uri.replace('file://', '') : filePath[0].uri,
        });
    ...
    const getUserData = async () =>{
      ...
      .then(responseData => {
          console.log("fetch Profile", responseData);
          const data = responseData && responseData.data?responseData.data[0]:null;
        ...
        <ScrollView>
          <View style={{ alignSelf: 'center' }}>
            <TouchableOpacity onPress={launchCamera} >
              <Image source={profile? {uri: profile.uri }: require('./user.png')} style={{ alignSelf: 'center', marginTop: 20, width: 100, height: 100, borderRadius: 50 }} />
              <Image source={require('./i_online.png')} style={{ position: 'absolute', bottom: 0, right: 0 }} />
            </TouchableOpacity>
          </View>
          <Text style={{ color: 'white', alignSelf: 'center', marginTop: 10 }}>{name}</Text>
          <StyledInput
            label={'Description'}
            type={'name'}
          ...
          <TouchableOpacity onPress={() => isLoading ?null : updateUser()}>
            <View style={{ backgroundColor: '#1455F5', borderRadius: 51, marginTop: 20, marginHorizontal: 10, marginBottom: 70, flexDirection: 'row', justifyContent: 'center' }}>
              {isLoading && <ActivityIndicator size="small" color="yellow" />}
              <Text style={{ color: 'white', fontSize: 18, alignSelf: 'center', paddingVertical: 10 }}>
