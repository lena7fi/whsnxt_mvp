...
function Home({ navigation, route, userData, accountType }) {
  ...
  const onePost = (data, key) => {
    ...
    let like_num = Number(comments[0]?.likes?comments[0]?.likes:0);
    let bookmark_num = Number(comments[0]?.bookmark?comments[0]?.bookmark:0);
    ...
    return (
      <SafeAreaView style={{ paddingHorizontal: 20, marginTop: 30 }} key={key}>
        <View
          style={{
            display: 'flex',
            flexDirection: 'row',
